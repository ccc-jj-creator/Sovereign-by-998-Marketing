...
export const useTypeformModal = () => {
  ...
  if (!context) {
    throw new Error('useTypeformModal must be used within TypeformModalProvider');
  }
  return context;
};
...
const TypeformContent: React.FC<{ onClose: () => void; source?: string }> = ({ onClose, source }) => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const payload = {
      ...
      source: source || 'Sovereign Wealth Systems - Popup',
  ...
  }, [handleNext, isCurrentValid, isSubmitting, isDisqualified, onClose]);
  ...
  if (isComplete) {
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-grow w-full bg-white/5 relative min-h-[400px]">
          <iframe
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-grow px-6 py-4 overflow-y-auto">
        {currentQuestion && (
          <div key={currentQuestion.id} className={animationClass}>
            <div className="mb-6">
              <h2 className="text-xl md:text-2xl font-serif text-white mb-2 leading-tight">
              ...
              <div className="grid grid-cols-2 gap-2">
                {currentQuestion.options?.map((option) => (
                  <button
                    ...
                    className={`p-3 md:p-4 border text-left transition-all duration-200 hover:border-wealth-gold/50 ${
                      formData[currentQuestion.field as keyof FormData] === option.value
                        ? 'border-wealth-gold bg-wealth-gold/10 text-white'
              ...
              <div className="space-y-2">
                {currentQuestion.options?.map((option) => (
                  <button
                    ...
                    type="button"
                    onClick={() => handleOptionSelect(currentQuestion.field, option.value)}
                    className={`w-full p-4 border text-left transition-all duration-200 flex items-start gap-3 ${
                      formData[currentQuestion.field as keyof FormData] === option.value
            ...
            {(currentQuestion.type === 'text' || currentQuestion.type === 'email' || currentQuestion.type === 'tel') && (
              <div>
                <input
                  ...
                  autoFocus
                  className="w-full bg-transparent border-b-2 border-white/20 text-white text-xl md:text-2xl py-3 focus:border-wealth-gold outline-none transition-colors placeholder:text-gray-600"
                />
                <p className="text-gray-600 text-xs mt-3">
                  Press <span className="text-gray-400 bg-white/5 px-2 py-0.5 rounded">Enter ↵</span> to continue
                </p>
              </div>
            )}
            ...
            {currentQuestion.type === 'checkbox' && (
              <label className="flex items-start gap-3 cursor-pointer group">
                <div
                  className={`w-5 h-5 border-2 flex-shrink-0 mt-0.5 transition-all flex items-center justify-center ${
            ...
            {currentQuestion.id === 'marketing_budget' && roi && formData.marketing_budget !== 'Under $3,000' && (
              <div className="mt-4 bg-gradient-to-r from-wealth-charcoal to-black border border-wealth-gold/30 p-4 animate-fadeIn">
                <div className="flex items-center gap-2 mb-2">
                  <svg className="w-3 h-3 text-wealth-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] text-gray-500 mb-1">Est. New HNW Clients</p>
                    <p className="text-lg text-white font-serif">{roi.clients} <span className="text-xs text-gray-500">/mo</span></p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-500 mb-1">Potential AUM Growth</p>
                    <p className="text-lg text-white font-serif">{roi.aum} <span className="text-xs text-gray-500">/yr</span></p>
                  </div>
                </div>
              </div>
            )}
            ...
            {isDisqualified && currentQuestion.id === 'budget_confirmation' && (
              <div className="mt-4 bg-red-900/20 border border-red-900/50 p-4 animate-fadeIn">
                <h3 className="text-red-400 font-bold text-sm mb-1">Application Paused</h3>
                <p className="text-gray-400 text-xs">
                  Our system requires a minimum investment of $3,000/month. When your budget allows, we'd be happy to continue.
                </p>
              </div>
            )}
          </div>
      ...
      <div className="px-6 py-4 border-t border-white/10 flex justify-between items-center">
        ...
        <button
          ...
          className={`px-6 py-2 text-xs font-bold tracking-wider uppercase transition-all flex items-center gap-2 ${
...
const TypeformModal: React.FC<{ isOpen: boolean; onClose: () => void; source?: string }> = ({ isOpen, onClose, source }) => {
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg mx-4 max-h-[90vh] bg-wealth-black border border-white/10 shadow-2xl flex flex-col animate-scaleIn">
        {/* Close button */}
        <button
          onClick={onClose}
        ...
        <div className="px-6 pt-6 pb-4 border-b border-white/10">
          <div className="inline-block bg-wealth-gold/10 border border-wealth-gold text-wealth-gold px-3 py-1 text-[10px] font-bold uppercase tracking-widest mb-3">
            Quick Application
          </div>
          <h2 className="text-xl font-serif text-white">Apply for Partnership</h2>
        </div>

        {/* Form content */}
        <div className="flex-grow overflow-hidden">
          <TypeformContent onClose={onClose} source={source} />
        </div>
      </div>
...
// Provider component
export const TypeformModalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <ModalContext.Provider value={{ openModal, closeModal, isOpen }}>
      {children}
      <TypeformModal isOpen={isOpen} onClose={closeModal} source={source} />
